...
void test_controller_poshold_stick_override_recaptures_hold_point()
{
  ...
  TEST_ASSERT_FLOAT_WITHIN(0.001f, Utils::toRad(model.config.level.angleLimit) * 0.5f,
                           model.state.setpoint.angle[AXIS_PITCH]);
...
void test_controller_poshold_limits_position_velocity_vector()
{
  ...
  TEST_ASSERT_FLOAT_WITHIN(0.001f, Utils::toRad(model.config.gps.posHoldMaxAngle),
                           model.state.setpoint.angle[AXIS_PITCH]);
}
