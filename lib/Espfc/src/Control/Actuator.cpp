...
bool Actuator::canActivateMode(FlightMode mode)
{
  switch (mode)
  {
    ...
    case MODE_POSHOLD:
      return _model.state.gps.present && _model.state.gps.fix && _model.state.gps.fixType >= 3 &&
             _model.state.gps.numSats >= _model.config.gps.minSats &&
             _model.config.gps.posHoldMaxHorizontalAccuracy > 0 && _model.config.gps.posHoldGpsTimeout > 0 &&
...
void Actuator::updateAirMode()
{
  ...
  if (armed && !_model.state.mode.airmodeAllowed &&
      _model.state.input.us[AXIS_THRUST] > 1400) // activate airmode in the air
  {
