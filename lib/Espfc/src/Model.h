...
#warning "Danger macro used ESPFC_DEV_PRESET_UNSAFE_ARMING"
  ...
  void begin()
  {
    ...
    state.boardAlignment.init(VectorFloat(Utils::toRad(config.boardAlignment[0]),
                                          Utils::toRad(config.boardAlignment[1]),
                                          Utils::toRad(config.boardAlignment[2])));
    onAccChange();
    ...
    for (size_t i = 0; i < AXIS_COUNT_RPY; i++)
    {
      ...
      {
        state.gyro.filter[i].begin(FilterConfig((FilterType)config.gyro.filter.type, config.gyro.dynLpfFilter.cutoff),
                                   gyroFilterRate);
      }
      else
      {
      ...
      state.attitude.filter[i].begin(FilterConfig(FILTER_PT1, state.accel.timer.rate / GYRO_FUSION_LPF_DIV),
                                     gyroFilterRate);
      for (size_t m = 0; m < RPM_FILTER_MOTOR_MAX; m++)
      {
        ...
        {
          int center =
              Utils::mapi(m * RPM_FILTER_HARMONICS_MAX + n, 0, RPM_FILTER_MOTOR_MAX * config.gyro.rpmFilter.harmonics,
                          config.gyro.rpmFilter.minFreq, gyroFilterRate / 2);
  ...
  void onAccChange()
  {
    state.trimRotation.init(
        VectorFloat{Utils::toRad(config.accel.trim[1]) * 0.1f, Utils::toRad(config.accel.trim[0]) * 0.1f, 0.0f});
  }
  ...
  void logStorageResult()
  {
    ...
    switch (_storageResult)
    {
      ...
      case STORAGE_NONE:
      default:
        logger.err().logln("EEPROM unknown result");
        break;
    }
#endif
  }
