...
int Input::begin()
{
  ...
  switch (_model.config.input.interpolationMode)
  {
    case INPUT_INTERPOLATION_AUTO:
      _model.state.input.interpolationDelta =
          std::clamp<uint32_t>(_model.state.input.frameDelta, 4000, 40000) * 0.000001f; // estimate real interval
      break;
  ...
  {
    if (_device)
      _filter[c].begin(FilterConfig(_device->needAverage() ? FILTER_FIR2 : FILTER_NONE, 1),
...
void FAST_CODE_ATTR Input::filterInputs(InputStatus status)
{
  ...
  const bool interpolation = _model.config.input.interpolationMode != INPUT_INTERPOLATION_OFF &&
                             _model.config.input.filterType == INPUT_INTERPOLATION;
