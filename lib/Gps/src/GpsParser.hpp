    ...
    switch (m.status)
    {
      ...
      case UBX_STATE_IDLE:
        if (c == UBX_SYNC0)
          ...
          m.status = UBX_STATE_IDLE;
        break;

      case UBX_STATE_SYNC:
        if (c == UBX_SYNC1)
          m.status = UBX_STATE_CLASS;
        else
          m.status = UBX_STATE_IDLE;
        break;
      ...
      case UBX_STATE_LEN1:
        ...
        if (m.length == 0)
          m.status = UBX_STATE_CRC0;
        else if (m.length >= 511)
          m.status = UBX_STATE_IDLE;
        else
          m.status = UBX_STATE_PAYLOAD;
        break;
