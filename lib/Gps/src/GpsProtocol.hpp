...
public:
  ...
  bool isReady() const
  {
    return status == UBX_STATE_READY;
  }
  bool isIdle() const
  {
    return status == UBX_STATE_IDLE;
  }

  bool isResponse(MsgId id) const
  {
    return msgId == id;
  }
  ...
  bool isAck() const
  {
    return msgId == UBX_ACK_ACK;
  }
  bool isAck(MsgId id) const
  {
    return isAck() && id == (payload[1] << 8 | payload[0]);
  }

  bool isNak() const
  {
    return msgId == UBX_ACK_NAK;
  }
  bool isNak(MsgId id) const
  {
    return isNak() && id == (payload[1] << 8 | payload[0]);
  }
};
...
public:
  ...
  bool isReady() const
  {
    return status == NMEA_STATE_READY;
  }
  bool isIdle() const
  {
    return status == NMEA_STATE_IDLE;
  }
};
...
constexpr uint32_t CFG_RATE_TIMEREF =
    0x20210003; // (1->0) Time reference (0-utc, 1-gps, 2-glonass, 3-BeiDou (18+), 4-Galileo (18+), 5-NavIC (29+))
...
public:
  ...
  uint32_t scanmode1; // Which SBAS PRN numbers to search for (bitmask). If all bits are set to zero, auto-scan (i.e.
                      // allvalid PRNs) are searched. Every bit corresponds to a PRN number
} __attribute__((packed));
...
constexpr uint32_t CFG_SBAS_PRNSCANMASK =
    0x50360006; // (0x72bc8 -> 0) Which SBAS PRN numbers to search for (bitmask). If all bits are set to zero, auto-scan
                // (i.e. allvalid PRNs) are searched. Every bit corresponds to a PRN number
...
public:
  ...
  uint8_t dynModel;     // Dynamic model, 0:portable, 2:stationary, 3:pedestrian, 4:automotive, 5:sea, 6:airbone<1g,
                        // 7:airbone<2g, 8: airbone<4g, 9:wrist watch, 10:motorbike, 11: lawnmower, 12: electric scooter
  ...
  uint8_t utcStandard; // UTC standard to be used, 0:auto, 3:USNaval Obs, 5: EURLabs, 6:Soviet, 7: NTSC (China), 8: NTPL
                       // (India)
  uint8_t reserved1[5];
} __attribute__((packed));

constexpr uint32_t CFG_NAVSPG_DYNMODEL =
    0x20110021; // (0->8) Dynamic platform model, 0:portable, 2:stationary, 3:pedestrian, 4:automotive, 5:sea,
                // 6:airbone<1g, 7:airbone<2g, 8: airbone<4g, 9:wrist watch, 10:motorbike, 11: lawnmower, 12: electric
                // scooter
...
constexpr uint32_t CFG_NAVSPG_INFIL_NCNOTHRS = 0x201100aa; // (0) Number of satellites required to have C/N0 above
                                                           // CFG-NAVSPG-INFIL_CNOTHRS for a fix to be attempted
constexpr uint32_t CFG_NAVSPG_INFIL_MINELEV =
    0x201100a4; // (5) Minimum elevation for a GNSS satellite to be used in navigation
...
constexpr uint32_t CFG_NAVSPG_UTCSTANDARD =
    0x2011001c; // (0) UTC standard 0:auto, 3:USNaval Obs, 5: EURLabs, 6:Soviet, 7: NTSC (China), 8: NTPL (India)
constexpr uint32_t CFG_MOT_GNSSSPEED_THRS = 0x20250038; // (0) GNSS speed threshold below which platform is considered
                                                        // as stationary (a.k.a. static hold threshold)
constexpr uint32_t CFG_MOT_GNSSDIST_THRS = 0x3025003b; //  (0) Distance above which GNSS-based stationary motion is exit
...
public:
  ...
  union
  {
    ...
    struct
    {
      ...
      uint8_t fullyResolved : 1; // 1 = UTC time of day has been fully resolved (no seconds uncertainty). Cannot be used
                                 // to check if time is completely solved.
  ...
  union
  {
    ...
    struct
    {
      ...
      uint8_t carrSoln : 2; // Carrier phase range solution status: 0: no carrier, 1: carrier with floating ambiguities,
                            // 2: with fixed ambiguities
    };
  ...
  union
  {
    ...
    struct
    {
      ...
      uint8_t lastCorrectionAge : 4; // Age of the most recently received differential correction: 0: ?, 1: 0-1, 2: 1-2,
                                     // 3: 2-5, 4: 5-10, 5: 10-15...
    };
