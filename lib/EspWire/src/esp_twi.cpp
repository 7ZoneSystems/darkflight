...
#if defined(ESP8266)
#define SDA_LOW()                                                                                                      \
  (GPES = (1 << esp_twi_sda)) // Enable SDA (becomes output and since GPO is 0 for the pin, it will pull the line low)
...
// #define SDA_LOW()   (GPIO.enable_w1ts.enable_w1ts = ((uint32_t) 1 << esp_twi_sda)) //Enable SDA (becomes output and
// since GPO is 0 for the pin, it will pull the line low)
static inline void SDA_LOW()
{
  GPIO.enable_w1ts.enable_w1ts = (uint32_t)1 << esp_twi_sda;
  GPIO.out_w1tc.out_w1tc = (uint32_t)1 << esp_twi_sda;
}
#define SDA_HIGH()                                                                                                     \
  (GPIO.enable_w1tc.enable_w1tc =                                                                                      \
       ((uint32_t)1 << esp_twi_sda)) // Disable SDA (becomes input and since it has pullup it will go high)
...
#elif defined(ESP32)
// #define SDA_LOW()   (GPIO.enable_w1ts = (1 << esp_twi_sda)) //Enable SDA (becomes output and since GPO is 0 for the
// pin, it will pull the line low)
static inline void SDA_LOW()
{
  GPIO.enable_w1ts = 1 << esp_twi_sda;
  GPIO.out_w1tc = 1 << esp_twi_sda;
}
#define SDA_HIGH()                                                                                                     \
  (GPIO.enable_w1tc = (1 << esp_twi_sda)) // Disable SDA (becomes input and since it has pullup it will go high)
...
#define SDA_LOW()                                                                                                      \
  (pinMode(esp_twi_sda, OUTPUT)) // Enable SDA (becomes output and since GPO is 0 for the pin, it will pull the line
                                 // low)
#define SDA_HIGH()                                                                                                     \
  (pinMode(esp_twi_sda, INPUT_PULLUP)) // Disable SDA (becomes input and since it has pullup it will go high)
...
static IRAM_ATTR bool esp_twi_write_stop(void)
{
  ...
  esp_twi_delay(esp_twi_dcount);
  SCL_HIGH();
  while (SCL_READ() == 0 && (i++) < esp_twi_clockStretchLimit)
    ; // Clock stretching
  esp_twi_delay(esp_twi_dcount);
  SDA_HIGH();
  esp_twi_delay(esp_twi_dcount);
...
static IRAM_ATTR unsigned char esp_twi_read_byte(bool nack)
{
  ...
  unsigned char bit;
  for (bit = 0; bit < 8; bit++)
    byte = (byte << 1) | esp_twi_read_bit();
  esp_twi_write_bit(nack);
  return byte;
}

unsigned char IRAM_ATTR esp_twi_writeTo(unsigned char address, unsigned char* buf, unsigned int len,
...
unsigned char IRAM_ATTR esp_twi_readFrom(unsigned char address, unsigned char* buf, unsigned int len,
                                         unsigned char sendStop)
{
  unsigned int i;
  if (!esp_twi_write_start()) return 4; // line busy
